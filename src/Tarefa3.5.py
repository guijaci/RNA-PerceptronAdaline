...
while True:
    ...
    training_set_inputs = hstack((training_set_inputs, ones((30, 1))))
    ...
    for iteration in range(1000000):
        ...
        delta = .1 * dot(training_set_inputs.T, error) / 30
        synaptic_weights += delta

    # Set to test the result network
    zArray = random.random(10) * 2 * pi
    validation_set_inputs = array(list(map(lambda x: [f1(x), f2(x), f3(x)], zArray)))

    # applying F function to f1, f2 and f3 (entries) F(z) = -pi + .565*f1(z) + 2.657*f2(z) + .674*f3(z)
    validation_set_outputs = dot(validation_set_inputs, array([[.565, 2.657, .674]]).T) - pi

    # appending 1's column to train for the constant
    validation_set_inputs = hstack((validation_set_inputs, ones((10, 1))))

    validation_output = dot(validation_set_inputs, synaptic_weights)
    validation_error = (validation_set_outputs - validation_output)
    avg_validation_error = validation_error.mean()

    print("Average validation error: ")
    print(avg_validation_error)
    if avg_validation_error < 1E-9:
        print("Training validated, now testing... \n")
        break
    else:
        print("Training not validated, retraining... \n")
...
# appending 1's column to train for the constant
test_set_inputs = hstack((test_set_inputs, ones((10, 1))))
